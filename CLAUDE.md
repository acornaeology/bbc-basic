...
**Version floor: dasmos ≥ 2.0.1, fantasm ≥ 0.17.2.** dasmos 2.0 classifies every cross-reference to an address as either a **direct** access (`lda addr` / `sta addr`) or an **indexing base** (`lda addr,X` / `sta addr,Y`, where the byte touched is `addr+reg` and the base byte itself is never read or written). The JSON is `schema_version: 2`: `references[]` entries are `{addr, kind, move_id?}` objects (not bare ints) and there is a new top-level `index_bases` section. Older fantasm misreads this schema — do not drop the floor. (Local tooling in `tools/` and `generate_readme.py` only reads `items`/`subroutines`/the project manifest, not the disassembly `references`, so it was unaffected.)
