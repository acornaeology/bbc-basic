...
**Version floor: dasmos ≥ 3.0.1, fantasm ≥ 0.18.0.** dasmos 2.0 introduced classifying every cross-reference to an address as either a **direct** access (`lda addr` / `sta addr`) or an **indexing base** (`lda addr,X` / `sta addr,Y`, where the byte touched is `addr+reg` and the base byte itself is never read or written), with `references[]` entries as `{addr, kind, move_id?}` objects (not bare ints) and a top-level `index_bases` section. The JSON is now `schema_version: 3` (dasmos 3.0 reshaped `expressions[]` to `{text, tree}` objects and added a top-level `macros` section — see the `pack_mnemonic_*` macros and the assembler-neutral `d.expr(...)` DSL in the driver). Older fantasm misreads these schemas — do not drop the floor. (Local tooling in `tools/` and `generate_readme.py` only reads `items`/`subroutines`/the project manifest, not the disassembly `references`, so it was unaffected.)

`d.index_base(addr, name, description=, group=, length=)` is the intention-revealing label for an address used **only** as an indexing base. It behaves like an optional label (the name still resolves the `,X`/`,Y` operand) but tags `access='indexed_base'`, which keeps the base **off** the fixed-location memory map and, for sub-&8000 addresses, lists it in the JSON `index_bases` section instead. Do **not** pass `access=` — `index_base` sets it. Note: **the JSON `memory_map` and `index_bases` sections are both sub-&8000 views**; an in-image ROM data table (≥&8000) converted to `index_base` is correctly tagged in the driver but does not appear in either section (it stays a label on its data item). Note also: **dasmos merges all labels at one address into a single label with one `access`** — you cannot have both a `memory_map` region label and an `index_base` label at the same address; pick one.
...
- It **under-reports low-page (page 0–2) bases** (schema-v2 `references[]` doesn't carry the kind for them, so fantasm excludes them conservatively). The dasmos `.asm` xref comment (`used as index base N times`) IS authoritative there — grep the `.asm` for it and cross-check against your driver labels.
